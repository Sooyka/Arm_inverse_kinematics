...
// arm.set_parameters{}
	...
	while (!viz.WindowShouldClose())
	{
		...
		{std::cout << end_pos.x << ", " << end_pos.y << ", " << end_pos.z << ", " << end_rot.x << ", " << end_rot.y << ", " << end_rot.z << '\n';
			...
			Coordinates target_change_cordinates = {end_rot.x * scale,end_rot.y * scale,end_rot.z * scale,end_pos.x * scale,end_pos.y * scale,end_pos.z * scale};
			Matrix4d target_change = exponential_coordinates_to_SE3(target_change_cordinates);
			// arm.inverse_kinematics(exponential_coordinates_to_SE3( effector_coordinates));
