...
Matrix4d Arm::effector_frame() const
{
    Matrix4d effector_frame = Matrix4d::Identity();
...
Matrix4d Arm::get_nth_segment_frame(int n) const
{
    Matrix4d segment_frame = Matrix4d::Identity();
    std::vector<Joint> joints = get_joints();
    std::vector<Segment> segments = get_segments();
    Joint joint;
    Segment segment;
...
Matrix4d Arm::get_nth_joint_frame(int n) const
{
    Matrix4d joint_frame = Matrix4d::Identity();
    std::vector<Joint> joints = get_joints();
    std::vector<Segment> segments = get_segments();
    Joint joint;
    Segment segment;
...
void Arm::set_parameters(const std::vector<double> &parameters)
{
    Joint old_joint;
...
std::vector<double> Arm::get_parameters() const
{
...
void Arm::inverse_kinematics(const Matrix4d &target_frame)
{
    // const Matrix4d target_frame = exponential_coordinates_to_SE3(coordinates);

    Matrix4d current_frame;
    Coordinates V_b_coordinates;
    // Matrix<double, 6, 1> V_b;
    MatrixXd V_b;
    V_b.resize(6, 1);
    ...
    std::vector<double> parameters;
    MatrixXd parameters_delta_vect;
    ...
    // Matrix4d Identityhmm = current_frame.inverse() * target_frame;
    V_b_coordinates = SE3_to_exponential_coordinates(current_frame.inverse() * target_frame);
    // V_b_coordinates = SE3_to_exponential_coordinates(target_frame);
    ...
    while (!precision_achieved && no_of_steps < max_no_of_steps)
    {
        ...
        MatrixXd Jacobian_pseudoinverse;
        ...
        {
            parameters[i] += parameters_delta_vect(i, 0) * 0.01;
        }
...
std::vector<Sn_theta> Arm::get_Sn_theta() const
{
    ...
    std::vector<double> length_along_segments;
    double length_summation = 0;
...
Matrix<double, 6, Dynamic> Arm::get_jacobian() const
{
    Matrix4d M_temp = Matrix4d::Identity();
    Segment segment;
    std::vector<Segment> segments;
    segments = get_segments();
    int no_of_segments = segments.size();
    ...
    const Matrix4d M = M_temp;

    Matrix<double, 6, Dynamic> Jacobian;
    int no_of_DoF = Arm::no_of_DoF();
    Jacobian.resize(6, no_of_DoF);
    ...
    Matrix<double, 6, 1> Jacobian_column = Matrix<double, 6, 1>::Zero();
    // Matrix<double, 6, 1> B_i;
    Matrix4d B_i;
    Matrix4d B_j;
    double theta_j;
...
Matrix4d Arm::Sn_product() const
{
    ...
    Sn_theta Sn_theta_i;
    Matrix4d Sn;
    Matrix4d Sn_product = Matrix4d::Identity();
    double theta;
    for (int i = 0; i < no_of_DoF; i++)
    {
        Sn_theta_i = Sn_theta_vec[i];
    ...
    // Matrix4d prod1 = exponential_coordinates_to_SE3(se3_to_exponential_coordinates(Sn * theta))*M;
    // Matrix4d prod2 = M*exponential_coordinates_to_SE3(se3_to_exponential_coordinates(M.inverse()*Sn*M * theta));
    // Matrix4d prod3 = M*exponential_coordinates_to_SE3(se3_to_exponential_coordinates(Sn_to_Bn(Sn, M) * theta));
